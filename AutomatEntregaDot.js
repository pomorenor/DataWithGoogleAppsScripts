...
function writeData() {
 ...
 // console.log(registeredEntries)
  ...
  console.log("updated quanitites")
...
//// Now we want to obtain a row with the date and the commentary 

var size = auxiliaryWithDates.getMaxColumns();
var infoWithDate = new Array(size);
const commentary = formatSheet.getRange("G6").getValue();
...
infoWithDate[size -1] = commentary;

console.log(infoWithDate);
//auxiliaryWithDates.appendRow(infoWithDate)
 

// Now we want to obtain the commentary and the size of the delivered imlement
...
/// Last developments for the stock part


function transposeRowToCol()
{
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const implementsSource = ss.getSheetByName("HojaAuxiliar");
  const placeToPlaceTheCol = ss.getSheetByName("RegistroCompras");

  const implements = implementsSource.getRange(1,1,1, implementsSource.getMaxColumns()).getValues()[0];

  implements.splice(0,7);
  const column = [];

  for (var i = 0; i < implements.length; i += 1){
    column.push([implements[i]]);
  }

  placeToPlaceTheCol.getRange(16,2,implements.length,1).setValues(column);
}
...
{
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const addColsToSheet = ss.getSheetByName("RegistroCompras");

  addColsToSheet.insertColumnsAfter(addColsToSheet.getMaxColumns(), addColsToSheet.getMaxColumns())
}




// And now we can start implementing what we want

// This functon works for updating the stock with the new implements

function updateStockWithBoughtImplements(element)
{
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const dataOrigin = ss.getSheetByName("RegistroCompras");
  const dataToFill = ss.getSheetByName("SeguimientoStock"); 


  const elementRange = element.range;
  const elementSheet = elementRange.getSheet();

  // We verify that we are using an element from the right sheet jeje

  if( elementSheet.getName() === "RegistroCompras"){

    const valueToAdd = element.value;
    const dataToFillRange = dataToFill.getRange(elementRange.getRow(), elementRange.getColumn());
    const dataToFillRangeValue = dataToFillRange.getValue();

    if(dataToFillRangeValue === ""){
      dataToFillRange.setValue(Number(valueToAdd))
    } else{
        dataToFillRange.setValue(Number(dataToFillRangeValue) + Number(valueToAdd))
    }
